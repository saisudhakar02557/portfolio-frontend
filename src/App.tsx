...
const skills = [
  {
    title: 'Backend',
    icon: ServerCog,
    items: ['Java 11', 'Java 17', 'Spring Boot', 'Spring MVC', 'Spring Data JPA', 'Spring Security', 'Hibernate', 'REST APIs', 'JWT', 'OAuth2'],
  },
  {
    title: 'Frontend',
    icon: Code2,
    items: ['React.js', 'Context API', 'TypeScript', 'JavaScript', 'HTML5', 'CSS3', 'Responsive UI', 'Vite'],
  },
  {
    title: 'Databases',
    icon: Database,
  ...
  },
  {
    title: 'Tools',
    icon: ShieldCheck,
    items: ['Git', 'GitHub', 'Maven', 'Postman', 'JIRA', 'IntelliJ IDEA', 'VS Code', 'JUnit 5', 'Mockito'],
  },
]
...
const projects = [
  {
    ...
    category: 'Full Stack',
    summary:
      'A role-based marketplace for farmers, managers, and admins with product inventory, ZIP-code discovery, order tracking, reviews, payments, and an AI assistant.',
    stack: ['React', 'Vite', 'Spring Boot', 'Java', 'MongoDB Atlas', 'JWT', 'Ollama'],
    highlights: [
      'Built workflows for Farmer, Manager, and Admin users with protected routes.',
      'Designed product, order, payment, review, and ZIP-code data models.',
      'Implemented marketplace behavior across REST endpoints and MongoDB collections.',
    ],
    link: null,
  },
  {
    ...
    summary:
      'A live freelance client website for a wealth advisory business, built with clear services, mobile-friendly layout, and direct inquiry paths.',
    stack: ['Responsive UI', 'JavaScript', 'SEO Structure', 'Client Delivery'],
    highlights: [
      'Created a professional website focused on credibility and quick contact actions.',
      'Improved the page structure for mobile visitors and fast scanning.',
  ...
  },
  {
    ...
    category: 'Full Stack',
    summary:
      'A shopping application with authentication, product catalog, cart state, order processing, and a React storefront.',
    stack: ['React.js', 'Spring Boot', 'MySQL', 'JWT', 'OAuth2', 'AWS EC2', 'AWS S3'],
    highlights: [
      'Designed modules for authentication, product catalog, cart, and order processing.',
      'Built a responsive storefront with role-based access and paginated product search.',
      'Prepared the project structure for cloud hosting with AWS services.',
    ],
    link: null,
  },
  {
    ...
    category: 'Backend',
    summary:
      'A rental operations backend for tenants, landlords, and admins with property, lease, and payment workflows.',
    ...
    highlights: [
      ...
      'Built middleware for authentication, authorization, validation, and ownership checks.',
      'Used MongoDB aggregation pipelines and indexed queries for reporting.',
    ],
    link: null,
  },
  {
    ...
    category: 'AI / ML',
    summary:
      'A computer vision project for road damage detection using image processing and model inference for maintenance reporting.',
    ...
    highlights: [
      ...
      'Structured reporting outputs for municipal maintenance review.',
    ],
    link: null,
  },
]
...
const experience = [
  {
    ...
    summary:
      'Worked on enterprise healthcare application features using Java, Spring Boot, React, and database-backed services for a multi-tenant platform.',
    bullets: [
      'Owned Spring Boot services and React.js features across 40+ bi-weekly agile sprints.',
      'Reduced high-volume API response latency to under 200ms across 50,000+ daily transactions.',
      'Improved system performance by 20% while supporting Java 11 to Java 17 modernization.',
      'Reduced MySQL query execution time by 35% through caching, lazy loading, indexing, and batch processing.',
      'Wrote unit and integration tests with JUnit, Mockito, and Spring Boot Test.',
    ],
  },
  {
    ...
    bullets: [
      'Delivered SVR Wealth as a live client website with business-focused content.',
      'Built applications using React, Spring Boot, MongoDB, MySQL, JWT security, and role-based workflows.',
      ...
      <main>
        ...
        <section className="section about-section" id="about" aria-labelledby="about-heading">
          ...
          <div className="about-grid">
            {[
              ['Engineering approach', 'Readable code, clean API contracts, practical database models, and frontend flows that make sense for real users.'],
              ['Problem solving', 'Strong DSA foundations for edge cases, data structures, performance, and maintainable implementation choices.'],
              ['Enterprise delivery', 'Accenture experience with multi-tenant healthcare applications, API tuning, React features, and code reviews.'],
              ['Professional interest', 'Seeking Java full stack, backend, and application developer roles.'],
            ].map(([title, detail]) => (
              <article key={title}>
                <h3>{title}</h3>
                <p>{detail}</p>
        ...
        <section className="section projects-section" id="projects" aria-labelledby="projects-heading">
          ...
          <div className="project-filters" aria-label="Filter projects by category">
            {categories.map((category) => (
              <button className={category === activeCategory ? 'active' : ''} key={category} type="button" onClick={() => setActiveCategory(category)}>
                {category}
              </button>
            ))}
          </div>
        ...
        <section className="section skills-section" id="skills" aria-labelledby="skills-heading">
          ...
          <img className="skill-icons" src={skillIconUrl} alt="Technology skill icons" />
        ...
        <section className="contact-section" id="contact" aria-labelledby="contact-heading">
          <div className="contact-copy">
            ...
            <div className="contact-lines">
              <a href={`mailto:${profile.email}`}><Mail size={17} aria-hidden="true" />{profile.email}</a>
              ...
              <a href={`tel:${profile.phone.replaceAll(' ', '')}`}><Phone size={17} aria-hidden="true" />{profile.phone}</a>
              <span><MapPin size={17} aria-hidden="true" />{profile.location}</span>
            </div>
            <div className="social-links">
              <a href="https://www.linkedin.com/in/venkata-kurre9132089972/" target="_blank" rel="noreferrer"><Network size={18} aria-hidden="true" />LinkedIn</a>
              <a href="https://github.com/saisudhakar02557/" target="_blank" rel="noreferrer"><GitBranch size={18} aria-hidden="true" />GitHub</a>
              <a href="https://leetcode.com/u/venkatakurre/" target="_blank" rel="noreferrer"><Cpu size={18} aria-hidden="true" />LeetCode</a>
            </div>
          </div>
          ...
          <form className="contact-form" onSubmit={handleContactSubmit}>
            ...
            <label>Name<input name="name" value={contactForm.name} onChange={handleContactChange} required placeholder="Your name" /></label>
            <label>Email<input name="email" type="email" value={contactForm.email} onChange={handleContactChange} required placeholder="you@example.com" /></label>
            <label>Company / context<input name="context" value={contactForm.context} onChange={handleContactChange} placeholder="Recruiter, client, project, etc." /></label>
            <label>Message<textarea name="message" value={contactForm.message} onChange={handleContactChange} required rows={5} placeholder="Tell me what you would like to discuss." /></label>
            <button className="button primary form-button" type="submit">Send message<Send size={17} aria-hidden="true" /></button>
